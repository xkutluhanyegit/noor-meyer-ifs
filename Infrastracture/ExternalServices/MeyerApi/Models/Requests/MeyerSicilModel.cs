using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace Infrastracture.ExternalServices.MeyerApi.Models.Requests
{
    public class MeyerSicilModel
    {
        public string Ad { get; set; }
        public string Soyad { get; set; }
        public string GirisTarih { get; set; }
    
        public string CikisTarih { get; set; }
        public string DogumTarihi { get; set; }
        public string SicilNo { get; set; }
        public string PersonelNo { get; set; }
        public string Cinsiyet { get; set; }
        public string Il { get; set; }
        public string Ilce { get; set; }
        public string Adres { get; set; }
        public string Ceptelefonu { get; set; }
        public string Telefon { get; set; }
        public string Email { get; set; }
        public string Firma { get; set; }
        public string Altfirma { get; set; }
        public string Bolum { get; set; }
        public string Direktorluk { get; set; }
        public string Gorev { get; set; }
        public string Pozisyon { get; set; }
        public string Yaka { get; set; }
        public string Kangrubu { get; set; }
        public string CardID { get; set; }
        public string Facilitycode { get; set; }

        public string Okod1 { get; set; }
        public string Okod2 { get; set; }
        public string Okod3 { get; set; }
        public string Okod4 { get; set; }
        public string Okod5 { get; set; }
        public string Okod6 { get; set; }
        public string Okod7 { get; set; }
        public string Okod8 { get; set; }
        public string Okod9 { get; set; }
        public string Okod10 { get; set; }
        public string Okod11 { get; set; }
        public string Okod12 { get; set; }
        public string Okod13 { get; set; }
        public string Okod14 { get; set; }
        public string Okod15 { get; set; }
        public string Okod16 { get; set; }
        public string Okod17 { get; set; }
        public string Okod18 { get; set; }
        public string Okod19 { get; set; }
        public string Okod20 { get; set; }
    }
}